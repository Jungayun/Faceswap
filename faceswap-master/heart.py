import cv2
skyImg = cv2.imread('jocker.jpg', -1)
rplogoImg = cv2.imread('heart2.png', -1)

## Image info
print
'===== Original Image Info ====='
print
'Sky Image : ', skyImg.shape
print
'RP Image  : ', rplogoImg.shape
...
for c in range(0, 3):
    skyImg[y_offset:y_offset + rp.shape[0], x_offset:x_offset + rp.shape[1], c] = \
        rp[:, :, c] * (rp[:, :, 2] / 255.0) + \
        skyImg[y_offset:y_offset + rp.shape[0], x_offset:x_offset + rp.shape[1], c] * (1.0 - rp[:, :, 2] / 255.0)
